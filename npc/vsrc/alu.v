    ...
    case (aluc)
      ...
      5'b10011: begin
        ...
        //   $display("next_pc ");
          ...
          $display("ecall  mepc = %h  mcause = %h mtvec = %h jump_pc = %h", mepc ,mcause,mtvec ,next_pc);
      ...
      5'b10101: begin
        ecall_pending = 1'b1;
        next_pc = mepc;
        $display("mret  mepc = %h  mcause = %h mtvec = %h jump_pc = %h", mepc ,mcause,mtvec ,next_pc);
    ...
    case (aluc)
      ...
      5'b10011: begin // ecall
        if (b[0] == 1'b0) begin
          mcause <= 32'hffffffff;
          mepc <= pc_current + 4;
          $display("ecall11  mepc = %h  mcause = %h mtvec = %h jump_pc = %h pc_current = %h", mepc ,mcause,mtvec ,next_pc ,pc_current);
